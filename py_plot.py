...
#?-------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        if len(csv_files) >= 1:
            ...
            for each in dict_keys :
                ...
                if dp.JSON['iterSplit']:
                    ...
                    with open(html_path + "_" + each + ".html", 'w', encoding='utf-8') as file: file.write(html_content)
                    file.close()
    ...
    def repo_3d(self, fileLog, simutil):
        ...
        var1, var2                                  = dp.JSON["Var1"], dp.JSON["Var2"]                                       # Varying X lists (X2 , X3 ...)
        ...
        relative_tol, absolute_tol                  = 0.001 , 1e-12                                                          # Relative tolerance / absolute tolerance for all close values skipping
        ...
        #?------------------------------------------------
        #?  Helper function to parse X values
        #?------------------------------------------------
        def parse_x_value(x_str):
            """Parse X value and flatten if it's a nested list like [[35], [75]]"""
            if isinstance(x_str, list):
                return x_str
            
            try:
                value = eval(x_str)
                if isinstance(value, list):
                    # Flatten nested lists: [[35], [75]] -> [35, 75]
        ...
        for i in range(1, 11):
            key = f"X{i}"
            if key in dp.JSON:
                value = parse_x_value(dp.JSON[key])
                if value != [0]:  # Check if not default [0]
        ...
        if plot_2D:
            ...
            for i in range(1, 11):
                key = f"X{i}"
                if key in dp.JSON and key not in [var1, var2]:
                    value = parse_x_value(dp.JSON[key])
                    if value != [0] and not dp.JSON["sweepNames"][int(dp.re.search(r'\d+', key).group())-1].startswith("X"):
