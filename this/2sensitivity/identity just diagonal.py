...
def generate_sensitivity_masks(nvars, perturbation_value, config_data):
    ...
    nominals = [original_Xs[i][0] for i in range(nvars) if original_Xs[i] != [0]]
    ...
    for i in range(10):
        if i < nvars:
            final_Xs.append(active_Xs[i])
        else:
            final_Xs.append(original_Xs[i])
...
your_config = {
    ...
    "X1": "[35]",      # First parameter with 2 points
    "X2": "[200]",    # Second parameter with 2 points
    "X3": "[11.5]",  # Third parameter with 2 points
    "X4": "[500]",    # Fourth parameter with 2 points
    "X5": "[66]",      # Fifth parameter with 2 points
    "X6": "[96369]", # Sixth parameter with 2 points
    "X7": "[0]",          # Inactive
    "X8": "[0]",          # Inactive
    "X9": "[0]",          # Inactive
    "X10": "[0]",         # Inactive
