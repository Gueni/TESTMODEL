...
simStruct = struct('ModelVars', struct('Common', struct('Control', struct('Targets', struct('Pout', 0, 'Vout', 0)), 'ToFile', struct('FileNameStandalone', ''), 'Load', struct('Front', struct('R_L', 0)), 'Thermal', struct('Twater', 0)), 'DCDC_Rail1', struct('Control', struct('Inputs', struct('Iin', 0, 'Pout', 0, 'Vout', 0, 'Vin', 0)))), 'SolverOpts', struct());
...
for sim = 1:size(data, 1)
    ...
    % Construct filename with simulation number and _sim.csv suffix
    filename = fullfile(D:\WORKSPACE\TESTMODEL\simulation_results\standalonefiletest, sprintf('_%03d_sim.csv', sim));
