...
simStruct = struct('ModelVars', struct('Common', struct('Control', struct('Targets', struct('Vout', 0, 'Pout', 0)), 'Thermal', struct('Twater', 0), 'Load', struct('Front', struct('R_L', 0)), 'ToFile', struct('FileNameStandalone', '')), 'DCDC_Rail1', struct('Control', struct('Inputs', struct('Vout', 0, 'Pout', 0, 'Iin', 0, 'Vin', 0)))), 'SolverOpts', struct());
...
for sim = 1:size(data, 1)
    ...
    % ===== Set output filename =====
    % Define base path using forward slashes
    base_path = 'D:\WORKSPACE\TESTMODEL\simulation_results\standalonefiletest';
    ...
    filename = strcat(base_path, sprintf('_%03d_sim.csv', sim));
