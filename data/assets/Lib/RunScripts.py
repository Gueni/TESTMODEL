...
class runScripts:
    ...
    def simInit(self):
        ...
        dp.mdlVars['DCDC_Average']  =   paramProcess.dcdcAverageModelCalculate(dp.mdlVars)                                                                          #!  update DCDC average model parameters

        if self.JS['limit_precision']:                                                                                                                              #!  limit precision of all model parameters
            dp.mdlVars              =   dp.PM.ParamProcess().limit_precision(dp.mdlVars , dp.mdl_precision)
            dp.slvOpts              =   dp.PM.ParamProcess().limit_precision(dp.slvOpts , dp.mdl_precision)
            dp.anlOpts              =   dp.PM.ParamProcess().limit_precision(dp.anlOpts , dp.mdl_precision)
        ...
        self.misc.InitializationCommands(self.obj.path,
                                         self.fileLog.resultfolder+"/"+"PLECS_MODEL_"+"standalone_"+self.JS['modelname'],
                                         dp.mdlVars,
                                         (dp.os.getcwd()).replace("\\","/")+"/Script/assets/InitializationCommands.m")                #!  write mdlVars as initialization commands in PLECS
        self.obj.ClearTrace(self.JS['modelname'],self.JS['scopes'])

    def simLog(self,OptStruct,Simulation=0):
        """This function creates simulation log file and writes the currently initialized simulation variables and parameters.

        Args:
            OptStruct (dict): dictionary housing all simulation variables
            Simulation (int, optional): simulation package number. Defaults to 0.
        """
        self.simutil.iterNumber    +=  1                                                                                                                            #!  increment iteration number
        itr                         =   self.simutil.iterNumber                                                                                                     #!  current iteration to be used in Focused Params

        self.plot.set_tab_dict(self.misc,OptStruct['ModelVars'])                                                                                                    #!  add model parameters values to report table
        self.fileLog.param_log(OptStruct,self.simutil.Threads,itr,self.simutil.Iterations,Simulation+1,self.simutil.Simulations)                                    #!  save simulation parameters to a log file
        ...
        self.plot.iter_param_unt.append(self.JS['paramUnts'])                                                                                                       #!  log focused parameters units
    ...
    def simMissing(self,iter,threads_vector,Threads):
        ...
        """
        MissingIter             =   postProcessing.findMissingResults(self.fileLog.resultfolder+"/CSV_TIME_SERIES",iter,threads_vector,Threads)                     #!  find missing iteration data file
        self.obj.OptStruct      =   []                                                                                                                              #!  reintialize empty optstruct
        self.obj.modelInit_path(self.JS['modelname'])                                                                                                               #!  reintialize simulation model to one thread
        self.obj.modelinit_opts(1,self.JS['parallel'])                                                                                                              #!  reintialize simulation model to one thread
